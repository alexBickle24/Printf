...
/*   Updated: 2024/11/23 21:58:53 by alex             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_put_unbr_base_c(unsigned int n, unsigned int base, int count, char flag)
{
    ...
    if (n >= base)
    {
        ...
        while(!dif && ((n % (x / y)) != 0))
        {
            count = count + ft_putbase_c("0123456789abcdef", base, 0, flag);
            y = y * base;
            dif = ((n / x) * y) - (n / (x / y));
        }
    ...
    {
        count = count + ft_putbase_c("0123456789abcdef", base, n, flag);
        return(count);
    }
}
...
int ft_put_ul_base_c(unsigned long n, unsigned long base, int count, char flag)
{
    ...
    {
        count = count + ft_putbase_c("0123456789abcdef", 16, n, flag);
        return(count);
    }
}
